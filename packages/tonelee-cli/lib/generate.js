...
module.exports = function generate(info, done) {
  let opts = {};

  // const metalsmith = Metalsmith(path.join(src, 'template'))
  const metalsmith = Metalsmith(path.join(info.src));

  const data = Object.assign(metalsmith.metadata(), {
    destDirName: info.name,
    inPlace: info.dest === process.cwd(),
    noEscape: true
  });
  ...
  if (!info.isCustom) {
    ...
    opts = getOptions(info.name, info.src, info.tmpName);
